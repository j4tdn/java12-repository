...
public class Ex10 {
	public static void main(String[] args) {
		...
		List<String> alpData = s.stream() //Stream<List<String>>
			.flatMap(Collection::stream) //Stream<String>  list -> list.stream() == Collection::stream
			.filter(item -> item.matches("[a-z]+"))
			.collect(Collectors.toList()); //List<String>
		CollectionUtils.printf(alpData);
