...
public class FileUtils {
	...
	public static File createFolder(String path) {
		...
		if(!file.exists()) {
			...
			isValid = creaFolder(file);
		}
		return isValid? file : null;
	}
	public static boolean creaFolder(File file) {
		boolean isValid= false;
		try {
			isValid= file.mkdir();
		} catch (Exception e) {
	...
	public static boolean[] create(String dirPath, int nof, Extension[] exts) {
		...
		for(int i=0;i< nof;i++) {
			String name = ""+System.currentTimeMillis() + i + exts[rd.nextInt(lenghth)].val();
	...
	public static void writeFile(Set<Integer> numbersSet, String path) {
		File file = new File(path);
		if (!file.exists()) {
			try {
				file.createNewFile();
